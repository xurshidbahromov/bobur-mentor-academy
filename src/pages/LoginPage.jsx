...
// Our actual logo from /public/favicon.svg
const Logo = ({ size = 36 }) => (
  <img src="/favicon.svg" alt="Bobur Mentor" width={size} height={size} style={{ objectFit: 'contain' }} />
)
...
function BrandPanel() {
  return (
    <div style={{
      ...
      background: 'linear-gradient(145deg, #1e3a8a 0%, #0f172a 55%, #0d1117 100%)',
      padding: '44px 52px',
      display: 'flex', flexDirection: 'column', justifyContent: 'space-between',
      position: 'relative', overflow: 'hidden',
    }}>
      {/* Decorative glows */}
      <div style={{ position: 'absolute', top: '-100px', right: '-80px', width: 380, height: 380, borderRadius: '50%', background: 'radial-gradient(circle, rgba(52,97,255,0.22) 0%, transparent 65%)', filter: 'blur(30px)', pointerEvents: 'none' }} />
      <div style={{ position: 'absolute', bottom: '-100px', left: '-60px', width: 360, height: 360, borderRadius: '50%', background: 'radial-gradient(circle, rgba(16,185,129,0.1) 0%, transparent 60%)', filter: 'blur(25px)', pointerEvents: 'none' }} />
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(false)
    if (error) setError("Email yoki parol noto'g'ri. Qaytadan urining.")
